...
def main_worker():
    ...
    for epoch in range(config['epoch_fit'] + config['epoch_debias']):
        ...
        if epoch >= config['epoch_fit']:
            # group_distance = H.abs().mean()
            group_distance = linalg.vector_norm(H, dim=1).mean()
            # group_distance = linalg.vector_norm(H)

        loss = F.binary_cross_entropy_with_logits(output[idx_train], labels[idx_train].unsqueeze(1).float())
        loss = loss + 0.5 * group_distance
        ...
        print("Epoch {}:".format(epoch),
              ...
              "ali: {:.4f}".format(linalg.vector_norm(H, dim=1).mean()),
              "std: {:.4f}".format(H.std(dim=1).mean()),
